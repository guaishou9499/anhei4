...
function team_ent.team_party(case, cur_main_quest)
    ...
    if team_nums == #team_info_list then
        ...
        local game_team_num = party_unit.member_list()
...
team_ent.get_my_info_table = function()
    local my_info = {
        ['角色ID'] = tostring(actor_unit.get_local_player_sysid()),
        ['角色名'] = local_player:c_name(),
...
function team_ent.get_team_job_list()
    local job_list = {}
    local job_info = common.split(user_set_ent['组队职业分配'], ',')
    for i = 1, #job_info do
        local job = common.split(job_info[i], '_')
        for j = 1, job[2] do
...
team_ent.set_my_info_team_redis = function(data_r, case)
    ...
    for i = 1, 100 do
        for j = 1, team_nums do
            local PATH = '暗黑4:内置数据:组队信息:' .. main_ctx:c_server_name() .. ':' .. case .. ':' .. i .. '-' .. j
        ...
        if team_idx > 0 then
            data_r['队伍位置'] = my_idx
            local PATH = '暗黑4:内置数据:组队信息:' .. main_ctx:c_server_name() .. ':' .. case .. ':' .. team_idx .. '-' .. my_idx
            redis_ent.update_data_to_redis(PATH, data_r) --写入角色信息到组队redis
            break
        end
    end
    return team_idx, my_idx, team_nums
...
team_ent.get_team_data = function(team_idx, team_nums, case)
    ...
    local my_data = {}
    local my_id = tostring(actor_unit.get_local_player_sysid())
    for i = 1, team_nums do
        local PATH = '暗黑4:内置数据:组队信息:' .. main_ctx:c_server_name() .. ':' .. case .. ':' .. team_idx .. '-' .. i
        local data = redis_ent.get_json_redis_by_path(PATH)
        if not table_is_empty(data) then
...
team_ent.is_team_leader = function(team_info_list, my_data, team_idx, case)
    local my_name = local_player:c_name()
    local my_id = tostring(actor_unit.get_local_player_sysid())
    if not my_data['队长名'] then
        ...
        if team_ent.can_do_leader(my_data['角色职业']) then
            ...
            local team_leader = ''
            for i = 1, #team_info_list do
                if team_info_list[i]['角色等级'] > max_level then
                    max_level = team_info_list[i]['角色等级']
                    team_leader = team_info_list[i]['角色名']
                end
            end
            if team_leader ~= '' and team_leader == my_name then
                --将队长信息写入所有同队队员信息下
                for i = 1, #team_info_list do
                    local data = { ['队长名'] = my_name, ['队长id'] = my_id}
                    local PATH = '暗黑4:内置数据:组队信息:' .. main_ctx:c_server_name() .. ':' .. case .. ':' .. team_idx .. '-' .. i
                    redis_ent.update_data_to_redis(PATH, data)
                end
                --打印所有队员信息
...
team_ent.leader_invite_member = function(team_data)
    --有队伍且队长为自己
    if party_unit.is_leader() or #party_unit.member_list() == 1 then
        --邀请其他队员组队
        local my_name = local_player:c_name()
        for i = 1, #team_data do
            ...
            if invited then
                party_unit.request(tonumber(id))
                decider.sleep(2000)
            end
        end
    else
...
team_ent.in_team_by_name = function(name)
    local ret_b = false
    local list = party_unit.member_list()
    for i = 1, #list do
        local member_info = list[i]
        if member_info.name == name then
            return true
        end
    end
    return ret_b
...
team_ent.member_wait_team = function(team_idx, my_idx, my_data)
    if #party_unit.member_list() > 1 then
        ...
        -- 判断是否为队长
        if party_unit.is_leader() then
            party_unit.transfer_leader(tonumber(my_data["队长id"]) )
            decider.sleep(2000)
        end
    else
        local btn_enter_game = ui_unit.get_control_byeid(0x02DEA0631BA49D96, 0xDFBE50774B5B6B62, true)
...
function team_ent.can_do_leader(job)
    ...
    local job_list = common.split(user_set_ent['队长职业'], ',')
    for i = 1, #job_list do
...
function team_ent.out_team()
    while decider.is_working() do
        ...
        if not party_unit.is_leader() then
            break
        end
        trace.output('退出队伍中')
        party_unit.leave()
